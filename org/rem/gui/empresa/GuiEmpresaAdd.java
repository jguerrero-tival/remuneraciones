...
public class GuiEmpresaAdd extends JDialog {
	 ...
	 */
	public GuiEmpresaAdd(JFrame owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory.createTitledBorder("Agregar Empresa"));
	...
	private JFormattedTextField getJFormattedTextFieldRutEmpresa() {
		if (jFormattedTextFieldRutEmpresa == null) {
			...
			jFormattedTextFieldRutEmpresa.addFocusListener(new FocusAdapter() {
				public void focusLost(FocusEvent evt) {
					jFormattedTextFieldRutEmpresaFocusLost(evt);
				}
			});
	...
	private void jButtonAddActionPerformed(ActionEvent evt) {
		if (this.jFormattedTextFieldRutEmpresa.getText().equals("__.___.___-_")) {
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Debe rellenar el campo \"Rut (Empresa)\".");
		} else if (!Rut.verificaRut(this.jFormattedTextFieldRutEmpresa.getText())) {
			Message.timerLabel(this.jLabelRespuesta, Color.red, "El campo \"Rut (Empresa)\" no es correcto.");
		} else if(this.jTextFieldRazonSocial.getText().trim().length()==0) {
			...
			this.respuesta(control.addEmpresa(
					this.jFormattedTextFieldRutEmpresa.getText().trim(),
					this.jTextFieldRazonSocial.getText().trim(),
					Palabra.setBooleanString(this.jComboBoxColegio.getSelectedItem().toString().trim()),
					this.jTextFieldActividadEconomica.getText().trim(),
					this.jTextFieldFono.getText().trim(),
					this.jTextFieldMail.getText().trim(),
					direccion,
					representante));
		}
	}
	...
	private void respuesta(int respuesta) {
		if (respuesta == control.respExito) {
			...
			Message.timerLabel(this.jLabelRespuesta, Color.red, "El registro estaba eliminado, se recupero exitosamente.");
			this.actualizar();
		}
	}
	
	public void actualizar() {
		//TODO Limpiar las casillas.
	...
	private void blanquear() {
		...
		this.jComboBoxColegio.setSelectedIndex(0);
		this.jTextFieldActividadEconomica.setText("");
		...
		this.jTextFieldComuna.setText("");
		this.jComboBoxRegion.setSelectedIndex(4);
		this.jTextFieldPoblacion.setText("");
		this.jTextFieldCerro.setText("");
		this.jFormattedTextFieldRutRepresentante.setText("");
		this.jTextFieldNombres.setText("");
		this.jTextFieldAPaterno.setText("");
		this.jTextFieldAMaterno.setText("");
		
		this.jTextFieldRazonSocial.setEditable(true);
		this.jComboBoxColegio.setEnabled(true);
		this.jTextFieldActividadEconomica.setEditable(true);
		this.jTextFieldFono.setEditable(true);
		...
		this.jTextFieldComuna.setEditable(true);
		this.jComboBoxRegion.setEnabled(true);
		this.jTextFieldPoblacion.setEditable(true);
		this.jTextFieldCerro.setEditable(true);
		this.jFormattedTextFieldRutRepresentante.setEditable(true);
		this.jTextFieldNombres.setEditable(true);
		this.jTextFieldAPaterno.setEditable(true);
		this.jTextFieldAMaterno.setEditable(true);
	}
	
	private void jFormattedTextFieldRutEmpresaFocusLost(FocusEvent evt) {
		Empresa empresa = control.getEmpresaBloqueada(this.jFormattedTextFieldRutEmpresa.getText());
		if (empresa!=null) {
			...
			this.jTextFieldRazonSocial.setEditable(false);
			this.jComboBoxColegio.setEditable(false);
			this.jTextFieldActividadEconomica.setEditable(false);
			this.jTextFieldFono.setEditable(false);
			...
			this.jTextFieldComuna.setEditable(false);
			this.jComboBoxRegion.setEditable(false);
			this.jTextFieldPoblacion.setEditable(false);
			this.jTextFieldCerro.setEditable(false);
			this.jFormattedTextFieldRutRepresentante.setEditable(false);
			this.jTextFieldNombres.setEditable(false);
			this.jTextFieldAPaterno.setEditable(false);
			this.jTextFieldAMaterno.setEditable(false);
		} else {
			this.blanquear();
		}
	}
