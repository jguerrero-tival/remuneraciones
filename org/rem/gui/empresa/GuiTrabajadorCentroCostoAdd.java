...
public class GuiTrabajadorCentroCostoAdd extends JDialog {

	private Boolean isColegio;
	 ...
	 */
	public GuiTrabajadorCentroCostoAdd(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory
					.createTitledBorder("Agregar Sueldo por Centro de Costo"));
	...
	private JPanel getJPanelTrabajadorCentroCosto2() {
		if (jPanelTrabajadorCentroCosto2 == null) {
			...
			jPanelTrabajadorCentroCosto2.add(getJComboBoxCentroCosto(), "1, 0, 3, 0");
	...
	private JPanel getJPanelTrabajador2() {
		if (jPanelTrabajador2 == null) {
			...
			jPanelTrabajador2.setPreferredSize(new java.awt.Dimension(445, 47));
	...
	private void jTextFieldHorasKeyTyped(KeyEvent evt) {
		Formato.formatLong(evt);
	}
	
	private void jButtonAddActionPerformed(ActionEvent evt) {
		if (this.jComboBoxCentroCosto.getSelectedIndex()==-1) {
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Debe seleccionar un \"Centro de Costo\".");
		...
		} else if (this.jTextFieldHoras.isEditable() && this.jTextFieldMonto.getText().trim().length() == 0) {
			...
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Debe rellenar el campo \"Horas Extraordinarias\".");
			 ...
			 if(this.jTextFieldHoras.isEditable()) {
				 this.respuesta(
					control.addTrabajadorCentroCosto(
							this.trabajadorCuenta, 
							new Long(this.jComboBoxCentroCosto.getSelectedItem().toString()),
	...
	public void actualizarDatos() {
		this.jComboBoxCentroCosto.setModel(control.getComboBoxModelCentroCosto(this.trabajadorCuenta));
	...
	public void actualizar(TrabajadorCuenta cuenta) {
		...
		this.isColegio = trabajadorCuenta.getEmpresaCuenta().getEmpresa().isColegio();
		this.actualizarDatos();
		this.limpiar();
	}
	...
	private JTextField getJTextFieldHoras() {
		if(jTextFieldHoras == null) {
			...
			jTextFieldHoras.addKeyListener(new KeyAdapter() {
				public void keyTyped(KeyEvent evt) {
	...
	private void updateEditableInputs() {
		if(isColegio && jComboBoxCentroCosto.getSelectedItem()!=null && ((Long)jComboBoxCentroCosto.getSelectedItem()).equals(new Long(100))) {
