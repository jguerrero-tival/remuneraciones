...
import org.rem.control.empresa.ControlTrabajadorNoImponible;
...
public class GuiTrabajadorNoImponibleRemove extends JDialog {

	private TrabajadorCuenta trabajadorCuenta;
	private TrabajadorNoImponible trabajadorNoImponible;
	private ControlTrabajadorNoImponible control = new ControlTrabajadorNoImponible();
	private JPanel jPanelEmpresa;
	private JPanel jPanelTrabajadorNoImponible2;
	private JTextField jTextFieldAPaterno;
	private JTextField jTextFieldNoImponible;
	...
	private JPanel jPanelTrabajadorNoImponible;
	 ...
	 */
	public GuiTrabajadorNoImponibleRemove(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory
					.createTitledBorder("Agregar NoImponible"));
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			...
			jPanel.add(getJPanelTrabajadorNoImponible(), "1, 3, 2, 3");
	...
	private JLabel getJLabelNumero() {
		if (jLabelNumero == null) {
			...
			jLabelNumero.setText("No Imponible : ");
		}
		return jLabelNumero;
	}
	...
	private JPanel getJPanelTrabajadorNoImponible() {
		if (jPanelTrabajadorNoImponible == null) {
			jPanelTrabajadorNoImponible = new JPanel();
			CardLayout jPanelTrabajadorNoImponibleLayout = new CardLayout();
			jPanelTrabajadorNoImponible.setLayout(jPanelTrabajadorNoImponibleLayout);
			jPanelTrabajadorNoImponible.setBorder(BorderFactory.createTitledBorder("No Imponible"));
			jPanelTrabajadorNoImponible.add(getJPanelTrabajadorNoImponible2(), "jPanelTrabajadorNoImponible2");
		}
		return jPanelTrabajadorNoImponible;
	}
	
	private JPanel getJPanelTrabajadorNoImponible2() {
		if (jPanelTrabajadorNoImponible2 == null) {
			jPanelTrabajadorNoImponible2 = new JPanel();
			TableLayout jPanelTrabajadorNoImponible2Layout = new TableLayout(
				new double[][] { { TableLayout.PREFERRED, TableLayout.FILL },
						{ TableLayout.PREFERRED, TableLayout.PREFERRED } });
			jPanelTrabajadorNoImponible2Layout.setHGap(5);
			jPanelTrabajadorNoImponible2Layout.setVGap(5);
			jPanelTrabajadorNoImponible2.setLayout(jPanelTrabajadorNoImponible2Layout);
			jPanelTrabajadorNoImponible2.add(getJLabelNumero(), "0, 0");
			jPanelTrabajadorNoImponible2.add(getJLabelMonto(), "0, 1");
			jPanelTrabajadorNoImponible2.add(getJTextFieldMonto(), "1, 1");
			jPanelTrabajadorNoImponible2.add(getJLabelHorasExtraordinarias(), "0, 2");
			jPanelTrabajadorNoImponible2.add(getJTextFieldHorasExtraordinarias(), "1, 2");
			jPanelTrabajadorNoImponible2.add(getJTextFieldNoImponible(), "1, 0");
		}
		return jPanelTrabajadorNoImponible2;
	}
	...
	private JTextField getJTextFieldNoImponible() {
		if (jTextFieldNoImponible == null) {
			jTextFieldNoImponible = new JTextField();
			jTextFieldNoImponible.setEditable(false);
		}
		return jTextFieldNoImponible;
	}
	
	private void jButtonRemoveActionPerformed(ActionEvent evt) {
		if (this.trabajadorCuenta!=null) {
			this.respuesta(
					control.removeTrabajadorNoImponible(
							this.trabajadorNoImponible
							));
		
		} else {
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Ya se eliminó el \"No Imponible\" ingresado.");
		}
	}
	...
	private void respuesta(int respuesta) {
		if (respuesta == control.respExito) {
			...
			Message.timerLabel(this.jLabelRespuesta, Color.red, "No existe el \"No Imponible\".");
	...
	public void actualizar(TrabajadorCuenta cuenta, TrabajadorNoImponible trabajadorDesc) {
		this.trabajadorCuenta = cuenta;
		this.trabajadorNoImponible = trabajadorDesc;
		...
		this.jTextFieldNoImponible.setText(this.trabajadorNoImponible.getNoImponible().getNombre().toString());
		this.jTextFieldMonto.setText(this.trabajadorNoImponible.getMonto().toString());
	}
	
	private void limpiar() {
		this.trabajadorNoImponible = null;
		this.trabajadorCuenta = null;
		this.jTextFieldNoImponible.setText("");
