...
public class GuiMutualAdd extends JDialog {
	 ...
	 */
	public GuiMutualAdd(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory
					.createTitledBorder("Agregar Mutual de Seguridad"));
	...
	private void jButtonAddActionPerformed(ActionEvent evt) {
		if (jTextFieldNombre.getText().trim().length() == 0) {
			...
			this.respuesta(control.addMutual(this.jTextFieldNombre.getText().trim()));
		}
	}
