...
public class GuiCajaAdd extends JDialog {
	 ...
	 */
	public GuiCajaAdd(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory
					.createTitledBorder("Agregar Caja de Compensación"));
	...
	private void jButtonAddActionPerformed(ActionEvent evt) {
		if (jTextFieldNombre.getText().trim().length() == 0) {
			...
			this.respuesta(control.addCaja(this.jTextFieldNombre.getText().trim()));
		}
	}
