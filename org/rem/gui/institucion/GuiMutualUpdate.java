...
public class GuiMutualUpdate extends JDialog {

	private ControlMutual control = new ControlMutual();
	private JPanel jPanelData;
	private JTextField jTextFieldNombre;
	private JLabel jLabel1;
	private JPanel jPanelInformation;
	private JComboBox jComboBoxMutual = new JComboBox();
	 ...
	 */
	public GuiMutualUpdate(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory.createTitledBorder("Modificar Mutual de Seguridad"));
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			...
			jPanel.add(jComboBoxMutual, "2, 1");
	...
	private void jButtonUpdateActionPerformed(ActionEvent evt) {
		if (jComboBoxMutual.getSelectedIndex()==-1) {
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Debe seleccionar un \"Mutual\".");
		}
			...
			this.respuesta(control.updateMutual( 
					this.jComboBoxMutual.getSelectedItem().toString(), 
	...
	private void actualizarCampos() {
		if (this.jComboBoxMutual.getSelectedIndex()!=-1) {
			this.jTextFieldNombre.setText( this.jComboBoxMutual.getSelectedItem().toString()); 
		}
	}
	
	public void actualizar() {
		this.jComboBoxMutual.setModel(control.getComboBoxModelMutualNoBloqueado());
		this.actualizarCampos();
	}
