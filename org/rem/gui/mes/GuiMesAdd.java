...
public class GuiMesAdd extends JDialog {
	 ...
	 */
	public GuiMesAdd(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory
					.createTitledBorder("Agregar Variables Mensuales"));
...
//						{ TableLayout.FILL, TableLayout.FILL } });
	...
	private JPanel getJPanelFecha() {
		if (jPanelFecha == null) {
			...
			jPanelFecha.add(getJSpinnerAnno());
			jPanelFecha.add(getJLabelMes());
			jPanelFecha.add(getJComboBoxMes());
		}
		return jPanelFecha;
	}
	...
	private JComboBox getJComboBoxMes() {
		if (jComboBoxMes == null) {
			...
			jComboBoxMes.setPreferredSize(new java.awt.Dimension(110, 20));
		}
		return jComboBoxMes;
	}
	...
	private JSpinner getJSpinnerAnno() {
		if (jSpinnerAnno == null) {
			...
			jSpinnerAnno.setPreferredSize(new java.awt.Dimension(75, 20));
			 formatSpinner =
			        ((JSpinner.DefaultEditor)jSpinnerAnno.getEditor()).getTextField();
	...
	private void jButtonAddActionPerformed(ActionEvent evt) {
		if (this.jComboBoxMes.getSelectedIndex()==-1) {
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Debe seleccionar un \"Mes\".");
		} else if (this.jTextFieldSueldoMinimo.getText().trim().length() == 0) {
			...
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Debe rellenar el campo \"Seguro Invalidez\".");			
			...
			System.out.println("VALOR : "+jSpinnerAnno.getValue());
			this.respuesta(control.addMes(
					Fecha.createDate((Long) ((JSpinner.DefaultEditor)jSpinnerAnno.getEditor()).getTextField().getValue(), this.jComboBoxMes.getSelectedItem().toString()), 
	...
	public void actualizar() {
		Date lastFecha = control.getFechaLastMes();
		Date newFecha = Fecha.getNextDate(lastFecha);
		((JSpinner.DefaultEditor)jSpinnerAnno.getEditor()).getTextField().setValue(Fecha.getYear(newFecha));
		this.jComboBoxMes.setSelectedItem(Fecha.getMonthString(newFecha));
		this.jTextFieldSueldoMinimo.setText(control.getSueldoMinimo(lastFecha).toString());
		this.jTextFieldUf.setText(control.getUf(lastFecha).toString());
		this.jTextFieldUtm.setText(control.getUtm(lastFecha).toString());
		this.jTextFieldAfpTope.setText(control.getPrevisionAfpTope(lastFecha).toString());
		this.jTextFieldApvTope.setText(control.getPrevisionApvTope(lastFecha).toString());
		this.jTextFieldSeguroInvalidez.setText(control.getPrevisionSeguroInvalidez(lastFecha).toString());
		this.jTextFieldMutualCotizacionBasica.setText(control.getMutualCotizacionBasica(lastFecha).toString());
		this.jTextFieldSaludCotizacionBasica.setText(control.getSaludCotizacionBasica(lastFecha).toString());
		this.jTextFieldSaludTope.setText(control.getSaludTope(lastFecha).toString());
		this.jTextFieldSeguroCesantiaPorcentaje.setText(control.getSeguroCesantiaPorcentaje(lastFecha).toString());
		this.jTextFieldSeguroCesantiaTope.setText(control.getSeguroCesantiaTope(lastFecha).toString());
		this.jTextFieldGratificacionPorcentaje.setText(control.getGratificacionPorcentaje(lastFecha).toString());
		this.jTextFieldGratificacionFactorAnual.setText(control.getGratificacionFactorAnual(lastFecha).toString());
	}
