...
import org.rem.control.haberDescuento.ControlDescuento;
...
public class GuiDescuentoUpdate extends JDialog {

	private ControlDescuento control = new ControlDescuento();  //  @jve:decl-index=0:
	private JPanel jPanelData;
	private JTextField jTextFieldNombre;
	private JLabel jLabel1;
	private JPanel jPanelInformation;
	private JComboBox jComboBoxDescuento = new JComboBox();
	 ...
	 */
	public GuiDescuentoUpdate(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory.createTitledBorder("Modificar Descuento"));
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			...
			jPanel.add(jComboBoxDescuento, "2, 1");
	...
	private JLabel getJLabelNombre() {
		if (jLabelNombre == null) {
			...
			jLabelNombre.setText("Descuento : ");
		}
		return jLabelNombre;
	}
	...
	private void jButtonUpdateActionPerformed(ActionEvent evt) {
		if (jComboBoxDescuento.getSelectedIndex()==-1) {
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Debe seleccionar un \"Descuento\".");
		}
			...
			this.respuesta(control.updateDescuento( 
					this.jComboBoxDescuento.getSelectedItem().toString(), 
	...
	private void jComboBoxDescuentoActionPerformed(ActionEvent evt) {
		this.actualizarCampos();
	}
	
	private void actualizarCampos() {
		if (this.jComboBoxDescuento.getSelectedIndex()!=-1) {
			this.jTextFieldNombre.setText( this.jComboBoxDescuento.getSelectedItem().toString()); 
		}
	}
	
	public void actualizar() {
		this.jComboBoxDescuento.setModel(control.getComboBoxModelDescuentoNoBloqueadoNoFijo());
		this.actualizarCampos();
	}
