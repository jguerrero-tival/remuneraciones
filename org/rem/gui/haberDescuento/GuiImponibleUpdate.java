...
import org.rem.control.haberDescuento.ControlImponible;
...
public class GuiImponibleUpdate extends JDialog {

	private ControlImponible control = new ControlImponible();  //  @jve:decl-index=0:
	private JPanel jPanelData;
	private JTextField jTextFieldNombre;
	private JLabel jLabel1;
	private JPanel jPanelInformation;
	private JComboBox jComboBoxImponible = new JComboBox();
	 ...
	 */
	public GuiImponibleUpdate(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory.createTitledBorder("Modificar Haber Imponible"));
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			...
			jPanel.add(jComboBoxImponible, "2, 1");
			jComboBoxImponible.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					jComboBoxImponibleActionPerformed(evt);
				}
			});
	...
	private JLabel getJLabelNombre() {
		if (jLabelNombre == null) {
			...
			jLabelNombre.setText("Imponible : ");
		}
		return jLabelNombre;
	}
	...
	private void jButtonUpdateActionPerformed(ActionEvent evt) {
		if (jComboBoxImponible.getSelectedIndex()==-1) {
			Message.timerLabel(this.jLabelRespuesta, Color.red, "Debe seleccionar un \"Imponible\".");
		}
			...
			this.respuesta(control.updateImponible( 
	...
	private void jComboBoxImponibleActionPerformed(ActionEvent evt) {
		this.actualizarCampos();
	}
	
	private void actualizarCampos() {
		if (this.jComboBoxImponible.getSelectedIndex()!=-1) {
			this.jTextFieldNombre.setText( this.jComboBoxImponible.getSelectedItem().toString()); 
		}
	}
	
	public void actualizar() {
		this.jComboBoxImponible.setModel(control.getComboBoxModelImponibleNoBloqueadoNoFijo());
		this.actualizarCampos();
	}
