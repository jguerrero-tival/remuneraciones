...
public class GuiImponibleAdd extends JDialog {

	private ControlImponible control = new ControlImponible();
	 ...
	 */
	public GuiImponibleAdd(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory
					.createTitledBorder("Agregar Haber Imponible"));
	...
	private void jButtonAddActionPerformed(ActionEvent evt) {
		if (jTextFieldNombre.getText().trim().length() == 0) {
			...
			this.respuesta(control.addImponible(this.jTextFieldNombre.getText().trim()));
		}
	}
