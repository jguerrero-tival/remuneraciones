...
public class GuiNoImponibleAdd extends JDialog {

	private ControlNoImponible control = new ControlNoImponible();
	 ...
	 */
	public GuiNoImponibleAdd(JDialog owner) {
		super(owner);
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.setBorder(BorderFactory
					.createTitledBorder("Agregar Haber No Imponible"));
	...
	private void jButtonAddActionPerformed(ActionEvent evt) {
		if (jTextFieldNombre.getText().trim().length() == 0) {
			...
			this.respuesta(control.addNoImponible(this.jTextFieldNombre.getText().trim()));
		}
	}
